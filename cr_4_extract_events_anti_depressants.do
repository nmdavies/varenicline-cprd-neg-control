...
if "`1'"=="med"{
	...
	foreach j in clinical referral test{
		...
		foreach f in `r(files)' {
			...
			save "/Volumes/varenicline_CPRD/tempdata/`f'_eventlist_`4'.dta", replace
			local files : list  f | files
			di "`files'"
			}
		}
	foreach i in `files'{
		append using "/Volumes/varenicline_CPRD/tempdata/`i'_eventlist_`4'.dta"
		rm "/Volumes/varenicline_CPRD/tempdata/`i'_eventlist_`4'.dta"
		}
	}
...
duplicates drop
save "/Volumes/varenicline_CPRD/tempdata/eventlist_`1'_`4'.dta", replace
end 
...
foreach i in nrt /* smokemed varenicline  buproprion  antidepressants antipsyc_stabalis cns_stimulants dementiameds hypnotics_anxiolytics lithium */ {
	extract_events prod "/Volumes/SSCM_shared/Studies/MHRA Project/New anti-depressants data/Suicides and anti-depressants2/" "Raw data/Stata/" `i'
	}
...
foreach i in /* autism bipolar current_smokers dementia depression	eatingdis hyperkineticdis  learningdis neuroticdis otherbehavdis persondis schizop smoke alcohol_misuse prob_selfharm drug_misuse TEMP_CHARLSON_INDEX TEMP_ALL_PSYCHIATRIC_ILLNESS*/ fractures{
	...
	save "7_negative_controls/working_data/eventlist/eventlist_med_`i'.dta"
