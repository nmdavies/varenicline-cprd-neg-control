...
ivreg2 cov_male (dr_varenicline=iv_*) rx_year_*, cluster(staffid) endog(dr_varenicline) partial(rx_year_*)
est sto iv_cov_male

reg cov_age dr_varenicline cov_male rx_year_* if iv!=., cluster(staffid)
est sto reg_cov_age
ivreg2 cov_age (dr_varenicline=iv_*) cov_male rx_year_*, cluster(staffid) endog(dr_varenicline) partial( cov_male rx_year_*)
est sto iv_cov_age
...
foreach i in `r(varlist)'{
	cap{
		...
		est sto reg_`i'
		ivreg2 `i' (dr_varenicline=iv_*)  cov_male , cluster(staffid) endog(dr_varenicline) partial( cov_male )
		est sto iv_`i'
		}
	}
...
foreach i in `r(varlist)'{
	cap{
		...
		est sto reg_`i'
		ivreg2 `i' (dr_varenicline=iv_*)  cov_male rx_year_*, cluster(staffid) endog(dr_varenicline) partial( cov_male rx_year_*)
		est sto iv_`i'
		}
	}
